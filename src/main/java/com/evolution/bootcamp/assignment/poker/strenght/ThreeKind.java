...
public class ThreeKind extends HandStrength {
    public ThreeKind(Hand hand, int base, int score) {
        super(hand, base, score);
    }

    /**
     * If tho hands are three kind on the same value so comparing the next cards from 4th card.
    ...
    public int compareTo(HandStrength other) {
        ...
        List<Integer> numbers = IntStream.range(0, cards.size()).dropWhile(i -> i < 3).mapToObj(cards::get)
                .mapToInt(Card::getIntVal).boxed()
                .collect(toList());
        List<Integer> otherNumbers = IntStream.range(0, otherCards.size()).dropWhile(i -> i < 3)
